...
import fractionalnftabi1 from '../contracts/NFT/FractionalNFT/FractionalNFTabi1';
import {fractionalnftdata1} from '../contracts/NFT/FractionalNFT/Fractionalnftdata1';
import fractionalnftabi2 from '../contracts/NFT/FractionalNFT/FractionalNFTabi2';
import {fractionalnftdata2} from '../contracts/NFT/FractionalNFT/Fractionalnftdata2';
import deployBond from '../../../assets/images/deploy-bond.jpeg'
import { BinanceFractionalNFTPost } from '../../../API/ApiFunctions';
...
const PostCard = () => {
 ...
 //deploy NormalNFT      
        const createdNFT = await fractionalnftabi1.deploy({
      
            data:fractionalnftdata1,
            arguments: ["FNFT","SFNFT"]        
            })
            .send({
            from: accounts[0],
            gas: 4796559,
          ...
          //Fractioanl deploy
          const createFractionalContract = await fractionalnftabi2.deploy({
      
            data:fractionalnftdata2,
            arguments: ["FNFT","SFNFT"]        
            })
            .send({
            from: accounts[0],
         ...
         setCreatedBond(createFractionalContract.options.address);

      await BinanceFractionalNFTPost(createdNFT.options.address,createFractionalContract.options.address);
            ...
            <Card className='card-dash border-0 d-block'>
                ...
                <div className="post-card-title mb-2 w-100 d-flex align-items-center">
                    ...
                    <div>
                        ...
                        {createdToken === "" ? <></> : <><center><h5>Created FractinalNFTContract1: <strong>{createdToken}</strong></h5></center><br/></>}
                ...
                <div className="post-card-body mb-3">
                   ...
                   <div>
                      {/* <h6 className='m-0'>Launchpad</h6> */}
                      {bondToken === "" ? <></> : <><center><h5>Created FractionalNFTContract2: <strong>{bondToken}</strong></h5></center><br/></>}
                      {/* <span className='d-block'>ELEM</span> */}
                  </div>
              </div> 
                 ...
                 <div className="post-card-body mb-3">
                     ...
                     <div>
                        {/* <h6 className='m-0'>Launchpad</h6> */}
                        {createdBond === "" ? <></> : <><center><h5>Created FractionalNFTContract: <strong>{createdBond}</strong></h5></center><br/></>}
                        {/* <span className='d-block'>ELEM</span> */}
                    </div>
                </div> 
                ...
                <div className="post-card-footer">
                    <div>
                    <ButtonLoad loading={loaderParticipate} className='w-100 btn-blue' onClick={Deploy}>Deploy FractionalNFT</ButtonLoad>                                   
                    </div>
