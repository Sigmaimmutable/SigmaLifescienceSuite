...
const token = {
   'X-API-Key': 'pOD5BAUCxq7InVPjo0sO01B0Vq4d7pD1ask5Ix43'}
    ...
    const treasuryHistory = async () =>
    {
        let amountTreasury = [];
        let timeTreasury = [];
        let totaltx = await indexClient.lookupAccountTransactions(dashboardDetails.dynamicStablecoinEscrowAddress).do();
        // console.log(totaltx);
        ...
        { 
           ...
           {
            if(totaltx['transactions'][i]['asset-transfer-transaction']['asset-id'] === tauID)
            {
                if(totaltx['transactions'][i]['asset-transfer-transaction']['receiver'] === dashboardDetails.dynamicStablecoinEscrowAddress)
                { 
        ...
        yaxis: {
            ...
            labels: {
                ...
                formatter: function (value) {
                    return value.toLocaleString() + " TAU"
                },
                style:{
                    colors: '#AAAAAA'
                }
