...
//     const indexClient = new algosdk.Indexer(token, baseServer, port);
    ...
    const treasuryHistory = async () =>
    {
        let amountTreasury = [];
        let timeTreasury = [];
        let totaltx = await indexClient.lookupAccountTransactions("K44D26OSBNXR4DOYT4E6X7NLUOWRVJYYHDKWCBRGUNTL3VF3TLZZSCVXQU").do();
        ...
        for(let i = 0; i < l; i++)
        {
            ...
            {
                if(totaltx['transactions'][i]['sender'] === "K44D26OSBNXR4DOYT4E6X7NLUOWRVJYYHDKWCBRGUNTL3VF3TLZZSCVXQU")
                {     
                    if( totaltx['transactions'][i]['asset-transfer-transaction']['amount'] )
                    {
        ...
        yaxis: {
            ...
            labels: {
                ...
                formatter: function (value) {
                    return value.toLocaleString() + " ELEM"
                },
                style:{
                    colors: '#AAAAAA'
                }
