...
import abi from '../contracts/Staking/abi';
import stakingabi from '../contracts/Staking/stakingabi'
import {ercdata} from '../contracts/Staking/data';
import {stakingdata} from '../contracts/Staking/stakingdata'
import deployBond from '../../../assets/images/deploy-bond.jpeg'
import { BinanceStakingPost } from '../../../API/ApiFunctions';
...
const PostCard = () => {
 ...
 //deploy DepositERC       
     ...
     //deploy BondERC       
     const RewardCreate = await abi.deploy({
      
        data:ercdata,
        arguments: ["StakeERC","SRERC"]        
        })
        .send({
        from: accounts[0],
        gas: 4796559,
      ...
      setCreatedTokenbond(RewardCreate.options.address);      
       
          localStorage.setItem("purchaseERC20",RewardCreate.options.address);
//Bond deploy
          const createStakeContract = await stakingabi.deploy({
      
            data:stakingdata,
            arguments: [createdtoken.options.address,RewardCreate.options.address]        
            })
            .send({
            from: accounts[0],
            gas: 4796559,
         ...
         setCreatedBond(createStakeContract.options.address);
        // let  erccontract = new web3.eth.Contract(abi, createLaunchpad.options.address);
        localStorage.setItem("createBondContract",createStakeContract.options.address);

          console.log("createBondContract",createStakeContract)
...
//approve
        let amount =  1000000000000000000 +"000000000000000000"; 
        await RewardCreate.methods.approve(createStakeContract.options.address,web3.utils.toBN((amount))).send({
            from:accounts[0],              
            // gas: 210000,
            //gasPrice: '20000000000'
            }).on('transactionHash',function(hash){      
...
//mint
            ...
            await RewardCreate.methods.mint(createStakeContract.options.address,web3.utils.toBN((amountmint))).send({
                from:accounts[0],              
                // gas: 210000,
                //gasPrice: '20000000000'
                }).on('transactionHash',function(hash){      
                ...
                await BinanceStakingPost(createStakeContract.options.address,createdtoken.options.address,RewardCreate.options.address);
            ...
            <Card className='card-dash border-0 d-block'>
                ...
                <div className="post-card-title mb-2 w-100 d-flex align-items-center">
                    ...
                    <div>
                        ...
                        {createdToken === "" ? <></> : <><center><h5>Created StakingToken: <strong>{createdToken}</strong></h5></center><br/></>}
                ...
                <div className="post-card-body mb-3">
                   ...
                   <div>
                      {/* <h6 className='m-0'>Launchpad</h6> */}
                      {bondToken === "" ? <></> : <><center><h5>Created RewardToken: <strong>{bondToken}</strong></h5></center><br/></>}
                      {/* <span className='d-block'>ELEM</span> */}
                  </div>
              </div> 
                 ...
                 <div className="post-card-body mb-3">
                     ...
                     <div>
                        {/* <h6 className='m-0'>Launchpad</h6> */}
                        {createdBond === "" ? <></> : <><center><h5>Created StakingContract: <strong>{createdBond}</strong></h5></center><br/></>}
                        {/* <span className='d-block'>ELEM</span> */}
                    </div>
                </div> 
                ...
                <div className="post-card-footer">
                    <div>
                    <ButtonLoad loading={loaderParticipate} className='w-100 btn-blue' onClick={Deploy}>Deploy StakingContract</ButtonLoad>                                   
                    </div>
