  ...
  const decodedResult = result.map(element => {
    ...
    const Donate =async (Pop_amount) => {
        ...
        //   const accounts = await myAlgoWallet.connect();
          ...
          let foreinAppId = [];
          foreinAppId.push(parseInt(78052725));
    ...
    const DonatePera =async (Pop_amount) => {
        ...
        //   let recv_escrow = escrow;
          // create unsigned transaction
          let foreinAsset = [];
          foreinAsset.push(parseInt(launchpadDetails['app1']['whiteAssetID']));
          let transaction1 = algosdk.makeApplicationNoOpTxnFromObject({
            from:sender, 
            suggestedParams: params, 
            appIndex: index, 
            appArgs: appArgs1,
            foreignAssets: foreinAsset
          })                    
...
{ 
    if(accountInfo['account']['apps-local-state'][j]['id'] === appID_global)
    {
        if(accountInfo['account']['apps-local-state'][j]['key-value'] === null)
        {
            ...
            for(let i = 0; i < 2; i++)
            {
            if(accountInfo['account']['apps-local-state'][j]['key-value'][i]['key'] === "bXlBbW91bnRHaXZlbg==")
            {
        ...
        <><ToastContainer position='bottom-right' draggable = {false} transition={Zoom} autoClose={4000} closeOnClick = {false}/></>
            ...
            <Card className='card-dash border-0 d-block'>
                ...
                <div className="post-card-footer">
                    ...
                    <ButtonLoad loading={whiteOpt} className='w-100' onClick={optinWhiteAsset}>White Asset Optin</ButtonLoad>
                ...
                <Modal.Body className='p-0'>
                    ...
                    <div className="d-flex align-items-start justify-content-between">
                        ...
                        <div className='d-flex ms-auto pt-2 flex-column align-items-end'>
                            ...
                            <div className="h6 mb-10">{(parseFloat(algoBalance)/1000000).toFixed(2) === 'NaN' ?<>0.00</> :(parseFloat(algoBalance)/1000000).toFixed(2)}&nbsp; ALGO</div>
                    ...
                    <div className="mb-10 d-flex flex-column align-items-end">
                        ...
                        {appOpt === false || assetOpt === false ? <ButtonLoad disabled loading={loaderParticipate} variant="primary" className='mb-10 py-1' onClick={()=>handleShowDonate()} style={{textTransform:"capitalize"}}>Participate</ButtonLoad> : <><ButtonLoad loading={loaderParticipate} variant="primary" className='mb-10 py-1' onClick={()=>handleShowDonate()} style={{textTransform:"capitalize"}}>Participate</ButtonLoad></>}
                    </div>
                    </div>
