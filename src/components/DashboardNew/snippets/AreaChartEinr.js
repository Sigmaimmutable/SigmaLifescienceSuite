...
const token = {
   'X-API-Key': 'pOD5BAUCxq7InVPjo0sO01B0Vq4d7pD1ask5Ix43'}
    ...
    const treasuryHistory = async () =>
    {
        let amountTreasury = [];
        let timeTreasury = [];
        let totaltx = await indexClient.lookupAccountTransactions(dashboardDetails.dynamicStablecoinEscrowAddressEinr).do();
        // console.log(totaltx);
        ...
        for(let i = 0; i < l; i++)
        { 
           if(totaltx['transactions'][i]['payment-transaction']) 
           {

           }
           else
           {
            if(totaltx['transactions'][i]['asset-transfer-transaction']['asset-id'] === einrID)
            {
                if(totaltx['transactions'][i]['asset-transfer-transaction']['receiver'] === dashboardDetails.dynamicStablecoinEscrowAddressEinr)
                { 
        ...
        yaxis: {
            ...
            labels: {
                ...
                formatter: function (value) {
                    return value.toLocaleString() + " EINR"
                },
                style:{
                    colors: '#AAAAAA'
                }
