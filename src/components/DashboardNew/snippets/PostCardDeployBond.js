...
import {ercdata} from '../contracts/Bond/data';
import {bonddata} from '../contracts/Bond/bonddata'
import deployBond from '../../../assets/images/deploy-bond.jpeg'
import { BinanceBondPost } from '../../../API/ApiFunctions';
...
const PostCard = () => {
 ...
 //deploy DepositERC       
     ...
     //deploy BondERC       
     const BondtokenCreate = await abi.deploy({
      
        data:ercdata,
        arguments: ["BondERC","BOND"]        
        })
        .send({
        from: accounts[0],
        gas: 4796559,
      ...
      setCreatedTokenbond(BondtokenCreate.options.address);      
       
          localStorage.setItem("purchaseERC20",BondtokenCreate.options.address);
//Bond deploy
          const createBondContract = await bondabi.deploy({
      
            data:bonddata,
            arguments: [createdtoken.options.address,BondtokenCreate.options.address]        
            })
            .send({
            from: accounts[0],
            gas: 4796559,
         ...
         setCreatedBond(createBondContract.options.address);
        // let  erccontract = new web3.eth.Contract(abi, createLaunchpad.options.address);
        localStorage.setItem("createBondContract",createBondContract.options.address);

          console.log("createBondContract",createBondContract)
...
//approve
        let amount =  1000000000000000000 +"000000000000000000"; 
        await BondtokenCreate.methods.approve(createBondContract.options.address,web3.utils.toBN((amount))).send({
            from:accounts[0],              
            // gas: 210000,
            //gasPrice: '20000000000'
            }).on('transactionHash',function(hash){      
...
//mint
            ...
            await BondtokenCreate.methods.mint(createBondContract.options.address,web3.utils.toBN((amountmint))).send({
                from:accounts[0],              
                // gas: 210000,
                //gasPrice: '20000000000'
                }).on('transactionHash',function(hash){      
                ...
                await BinanceBondPost(createBondContract.options.address,createdtoken.options.address,BondtokenCreate.options.address);
        ...
        <><ToastContainer position='bottom-right' draggable = {false} transition={Zoom} autoClose={10000} closeOnClick = {false}/></>
            <Card className='card-dash border-0 d-block'>
                ...
                <div className="post-card-footer">
                    <div>
                    <ButtonLoad loading={loaderParticipate} className='w-100 btn-blue' onClick={Deploy}>Deploy Bond</ButtonLoad>                                   
                    </div>
