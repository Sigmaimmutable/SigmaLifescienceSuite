...
const AreaChartNFT = () => {
    ...
    //console.log("TresPrint",treasury)
...
//     const indexClient = new algosdk.Indexer(token, baseServer, port);
    ...
    const treasuryHistory = async () =>
    {
        let amountTreasury = [];
        let timeTreasury = [];
        let totaltx = await indexClient.lookupAccountTransactions(config.feesescrow).do();
        let totaltxCount = totaltx['transactions']['length'];
        //console.log("TotalArea",totaltxCount)
        let l;
        if(totaltxCount <= 100)
        {
            l = totaltxCount;
        ...
        for(let i = 0; i < l; i++)
        {
            ...
            if(totaltx['transactions'][i]['tx-type'] === "pay")
            {
                //console.log("Area1",totaltx['transactions'][i]);
                if(totaltx['transactions'][i]['payment-transaction']['receiver'] === config.feesescrow)
                {  
                       //console.log("Area",totaltx['transactions'][i]['payment-transaction']['amount']);
                    if( totaltx['transactions'][i]['payment-transaction']['amount'] !== null)
                    {                        
                        amountTreasury.push(totaltx['transactions'][i]['payment-transaction']['amount'])
                        setTreasury(amountTreasury);
                        timeTreasury.push(totaltx['transactions'][i]['round-time'])
                        setTreasuryTime(timeTreasury);
                        //console.log("amounyT",amountTreasury);
                        j++;
                        // console.log("count");
                    }
                    else
        ...
        yaxis: {
            ...
            labels: {
                ...
                formatter: function (value) {
                    return value.toLocaleString() + " ALGO"
                },
                style:{
                    colors: '#AAAAAA'
                }
