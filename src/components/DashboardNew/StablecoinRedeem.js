...
import redeemDetails from '../Dashboard/stablecoin.json';
...
const StablecoinRedeem = () => {
    ...
    const[globalcr,setglobalcr] = useState("");
    ...
    const[redeempricethreshold,setRPT] = useState("")
    ...
    const firstrender = async()=>{
        ...
        setRPT(await poolcontract.methods.redeem_price_threshold().call())
console.log("RPT",redeempricethreshold);
console.log("fraxprice",fraxprice);

        let global = await elemcontract.methods.global_collateral_ratio().call();
        ...
        if(fraxprice >=redeempricethreshold){
            setIsOpens(true);
    ...
    const amountSet = async(value)=>{
        seteneteredamount(value);
        // let k = (parseInt(value*1000000000000000000))*1000000/fxsprice;
        // setfxsout(k)
        // let l = (parseInt(value*1000000000000000000))*0.975*globalcr/1000000;
        // let fincollateral = await poolcontract.methods.getFRAXInCollateral(0,web3.utils.toBN(parseInt(l))).call();
        // setcolout(fincollateral)
        // console.log("value",k,fincollateral);
        let frax_after_fee = (parseInt(value*1000000000000000000))*((1000000-(collateralinfo.redemption_fee))/(1000000));
        
       let  collat_out = (frax_after_fee*globalcr*collateralinfo.price)/(1000000000000); // PRICE_PRECISION ^2 + missing decimals
       setcolout(collat_out)  
       let fxs_out = frax_after_fee*(1000000-(globalcr))/(fxsprice);
       setfxsout(fxs_out)
         console.log("value",fxs_out,parseFloat(collat_out),frax_after_fee);
    }

    const redeem = async(f,c)=>{
        handleShowRedeem();
        const accounts = await  web3.eth.getAccounts();
        try{
            await poolcontract.methods.redeemFrax(0,web3.utils.toBN(parseInt(enteredamount*1000000000000000000)),web3.utils.toBN(parseInt(f-10000)),web3.utils.toBN(parseInt(c-10000))).send({
                from:accounts[0],              
                // gas: 210000,
                //gasPrice: '20000000000'
                }).on('transactionHash',function(hash){      
                ...
                firstrend()
                firstrender()
                await poolcontract.methods.collectRedemption(0).send({
                    from:accounts[0],              
                    // gas: 210000,
                    //gasPrice: '20000000000'
                    });
    ...
    const maxcall= async()=>{
        amountSet(elembalance/1000000000000000000)
    }
    const toastDiv = (txId) =>
    (
        <div>
    ...
    return (
        ...
        <Layout>
                    ...
                    <Col md={10} lg={7} className="mb-4">
                        ...
                        <Card className='card-dash d-block border-0 mb-4'>
                            ...
                            <Tabs defaultActiveKey="mint" className='dashboard-tabs' id="tab-example-1">
                                <Tab eventKey="mint" title="Redeem cDAI">
                                    ...
                                    <div className="group-row mb-20">
                                        <Row>
                                            ...
                                            <Col sm={7}>
                                                <div className="input-group-max px-3 input-group-max-lg w-100">
                                                    <input readonly disabled type="number" placeholder='0.00' className='form-control' value={parseFloat(fxsout/1000000000000000000).toFixed(6)} />
                                                </div>
                                            </Col>
                                        </Row>
                                    </div>                                    
                                    <div className="group-row">
                                        <Row>
                                            ...
                                            <Col sm={7}>
                                                <div className="input-group-max px-3 input-group-max-lg w-100">
                                                    <input readonly disabled type="number" placeholder='0.00' className='form-control' value={parseFloat(colout/1000000000000000000).toFixed(6)}/>
                                                </div>
                                            </Col>
                                        </Row>
                                    </div>
                                    ...
                                    <div className="mb-20">
                                        ...
                                        <div className="d-flex mb-1 align-items-center justify-content-between text-md">
                                            <span> </span>
                                            <strong className='font-semibold'>Redemption fee: {collateralinfo.redemption_fee/10000}%</strong>
                                        </div>
                                        {/* <div className="d-flex mb-1 align-items-center justify-content-between text-md">
                                    ...
                                    <Row className='flex-nowrap align-items-center gx-3'>
                                        <Col>
                                            <ButtonLoad loading={loadRedeem} className='btn w-100 btn-blue' onClick={()=>redeem(fxsout,colout)} >
                                                Redeem cDAI
                                            </ButtonLoad>
                                        </Col>
                                        {/* <Col>
                                ...
                                <div className="group-row">
                                    ...
                                    <Row className='flex-nowrap align-items-center gx-3'>
                                        <Col>
                                            <ButtonLoad loading={loadRedeem} className='btn w-100 btn-blue' >
                                                Redeem EINR
                                            </ButtonLoad>
                                        </Col>
            ...
            <div className="pt-xl-0 pt-4">   
                ...
                <Modal.Body>
                { daiapproved === false ? <ButtonLoad loading={loadMint} variant='primary' className='d-flex p-3 mb-20 justify-content-between w-100 align-items-center' onClick={()=>approvedai()} >
                        <span className='text-white'>1.Approve DAI</span>
                    </ButtonLoad> : <></>}
                    { fraxapproved === false ? <ButtonLoad loading={loadAppOpt} variant='primary' className='d-flex p-3 mb-20 justify-content-between w-100 align-items-center' onClick={()=>approvefrax()}  >
                        <span className='text-white'>2. Approve cDAI </span>
                    </ButtonLoad> : <></>}   
                    { elemapproved === false ? <ButtonLoad loading={loadelemopt} variant='primary' className='d-flex p-3 mb-20 justify-content-between w-100 align-items-center' onClick={()=>approveelem()}  >
...
{isOpens === true ?(<>
        ...
        <div className="pt-xl-0 pt-4">   
            ...
            <Modal.Header>
                <Modal.Title> Frax price is too high to redeem </Modal.Title>
            </Modal.Header>
            <Modal.Header>
                <Modal.Title>  </Modal.Title>
            </Modal.Header>
